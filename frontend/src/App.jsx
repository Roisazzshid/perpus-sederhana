        ...
        {rows.map((r) => (
          <tr key={r.id}>
            ...
            <td>
              ...
              <button className="btn danger" onClick={() => onDelete(r.id)}>Del</button>
            </td>
          </tr>
        ))}
        {!rows.length && (
...
export default function App() {
  ...
  const api = useMemo(() => {
    ...
    return {
      listBooks: () => fetch(`${API_BASE}/books`).then(withJson),
      upsertBook: async (payload, id) => {
      ...
      deleteBook: (id) => fetch(`${API_BASE}/books/${id}`, { method: 'DELETE' }).then(withJson),

      listMembers: () => fetch(`${API_BASE}/members`).then(withJson),
      upsertMember: async (payload, id) => {
        const method = id ? 'PUT' : 'POST';
      ...
      deleteMember: (id) => fetch(`${API_BASE}/members/${id}`, { method: 'DELETE' }).then(withJson),

      listLoans: () => fetch(`${API_BASE}/loans`).then(withJson),
      createLoan: (payload) =>
      ...
      returnLoan: (id) =>
        fetch(`${API_BASE}/loans/${id}/return`, { method: 'PUT' }).then(withJson)
    };
  }, []);
          ...
          <div className="card">
            ...
            <table className="table">
              ...
              <tbody>
                ...
                {!loans.length && (
                  <tr>
                    <td colSpan={7} style={{ textAlign: 'center', color: '#777' }}>Tidak ada peminjaman</td>
                  </tr>
                )}
              </tbody>
            </table>
